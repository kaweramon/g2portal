...
public class UploadNfeTask {
	...
	public UploadNfeTask(JLabel labelTaskStatus) {
	...
	public void uploadNfeXmls() {
		timer.schedule(new UploadNfeTimerTask(appsBean, labelTaskStatus), 20 * 1000);
	}
	
	public void uploadNFCeXmls() {
		this.timer = new Timer(true);
		timer.schedule(new UploadNfCeTimerTask(appsBean, labelTaskStatus), 10 * 1000);
	}
}
...
class UploadNfeTimerTask extends TimerTask {
	...
	public UploadNfeTimerTask(AppsBean appsBean, JLabel labelTaskStatus) {
		this.appsBean = appsBean;
		this.labelTaskStatus = labelTaskStatus;
		this.uploadNfeTask = new UploadNfeTask(labelTaskStatus);
	}
	...
	public void run() {
		...
		DropboxUploadFile dropboxUpload = new DropboxUploadFile();
		for (SightSale sightSale : listSightSale) {
			if (sightSale.getNfeStatus().equals("Enviada") && 
				...
				if (file != null) {
					...
					String pathDropbox = "/" + getCleanCnpj(appsBean) + "/" + 
							month + calendar.get(Calendar.YEAR) + "/" + file.getName();
...
class UploadNfCeTimerTask extends TimerTask {
	...
	public UploadNfCeTimerTask(AppsBean appsBean, JLabel labelTaskStatus) {
		this.appsBean = appsBean;
		this.labelTaskStatus = labelTaskStatus;
		this.cleanCnpj = getCleanCnpj();
		this.calendar = Calendar.getInstance();
		this.deleteNfsTask = new DeleteNfeAndNfceCanceledTask(labelTaskStatus, appsBean);
	}
	...
	public void run() {
		...
		for (QuickSell quickSell : listQuickSell) {
			...
			if (file != null) {
				String pathDropbox = "/" + this.cleanCnpj + "/NFCe_XML/" + getMonthAndYearPath(calendar) + "/" + 
						quickSell.getNfceKey() + ".xml";
				if (dropboxUpload.uploadFile(file, labelTaskStatus, pathDropbox))
